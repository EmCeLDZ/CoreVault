...
using CoreVault.Application.DTOs;
...
public class ApiIntegrationTests : IAsyncLifetime
{
    ...
    public async Task Get_ShouldReturnNotFound_WhenKeyDoesNotExist()
    {
        ...
        var debugResponse = await _client.GetAsync("/api/kv/keyvalue/debug")!;
        ...
        var response = await _client.GetAsync("/api/kv/keyvalue/public/nonexistent");
    ...
    public async Task Post_And_Get_ShouldReturnSavedValue()
    {
        ...
        var testGetResponse = await _client.GetAsync("/api/kv/keyvalue")!;
        if (testGetResponse != null)
        {
            Debug.WriteLine($"Test GET Status: {testGetResponse.StatusCode}");
            Debug.WriteLine("Test GET completed successfully");
        }
        ...
        try
        {
            var postResponse = await _client.PostAsJsonAsync("/api/kv/keyvalue", postData);
